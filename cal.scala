...
object smallCal{
  ...
  def indexRMAD(
    ...
    println(f"Calc took $initTimeInSeconds%.3f seconds.")
...
def main(args: Array[String]): Unit ={
    ...
    //val datap = spark.read.format("parquet").load( args(0) )
    ...
    val withIndex = rows.zipWithIndex

    val indexKey = withIndex.map{ case (k,v) => (v,k) }

    val b = indexKey.lookup(rowLargestSum)
